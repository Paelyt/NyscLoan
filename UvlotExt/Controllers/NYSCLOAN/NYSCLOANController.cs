    ...
    public class NYSCLOANController : Controller
    {
        ...
        public ActionResult NYSCLoanAppForm(DataAccessA.Classes.LoanApplication laobj)
        {
            try {
              ...
              //  ViewBag.nTen = _DR.GetAllTenure();

               // var a = _DR.GetAllTenure();
                UvlotAEntities uvdb = new UvlotAEntities();

                /*  var Service = (from b in uvdb.NYSCLoanSetUps select a).ToList();
                  var serv = (from v in uvdb.NYSCLoanSetUps select a).ToList();*/
                //var ub = uvdb.NYSCLoanSetUps.ToList();
                //var cd = uvdb.NyscMaritalStatus.ToList();
                int val = 0;
