...
namespace UvlotExt.Controllers
{
    public class AdminAController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            var ApplicationDropOutRate = GetTotalApplicationAbandonRate();
            TempData["ApplicationDropOutRate"] = ApplicationDropOutRate;
            var DropOutRate = GetDropOutRate();
            TempData["DropOutRate"] = DropOutRate;
            ViewBag.RepaymentEfficiency = RepaymentEfficiency();
            ViewBag.GetTotalAcceptanceRate = GetTotalAcceptanceRate();
        ...
        public float RepaymentEfficiency()
        {
            ...
            try
            {
                var d = DateTime.Today.Month;
                var b = DateTime.Today.Year;
                var Yr = (from a in db.NYSCLoanLedgers where a.PaymentFlag != 1 && a.DueDate != null   select a.DueDate.Value.Year).ToList();
                var Dy = (from a in db.NYSCLoanLedgers where a.PaymentFlag != 1 && a.DueDate != null select a.DueDate.Value.Month).ToList();
                var DefaultLoans = (from a in db.NYSCLoanLedgers where a.PaymentFlag != 1 && a.DueDate.Value.Month < DateTime.Today.Month && a.DueDate.Value.Year <= DateTime.Today.Year select a.Credit).Sum();
                                    //&& a.DueDate.Value.Month < DateTime.Today.Month && a.DueDate.Value.Year < DateTime.Today.Year select a.Credit).Sum();
        ...
        public float GetTotalAcceptanceRate()
        {
            ...
            try
            {
                var AppDisbursed = (from a in db.NyscLoanApplications where a.NYSCApplicationStatus_FK == 5 select a).Count();
                var AppSubmitted = (from a in db.NyscLoanApplications where a.NYSCApplicationStatus_FK == 1 select a).Count();

                float  AppDisburse = AppDisbursed;
                float AppSubmitteds = AppSubmitted;
                float Result = AppDisburse / AppSubmitteds;
        ...
        public float GetTotalApplicationAbandonRate()
        {
          try
            {
                int DateCap = 15;
                //var DC = (from a in db.NyscLoanApplications where a.RefNumber == "NY15922530653495" select a.DateCreated).FirstOrDefault();
                //var currentDate = DateTime.Now;
                //var DiffDATE = System.Data.Entity.DbFunctions.DiffDays(currentDate,DC);
                //var AppDisbursed = (from a in db.NyscLoanApplications where a.NYSCApplicationStatus_FK == 11 && a.IsVisible == 1 && System.Data.Entity.DbFunctions.DiffDays(DateTime.Now,a.DateCreated) > DateCap select a).Count();

                var AppDisburseds = (from a in db.NyscLoanApplications where DateCap > System.Data.Entity.DbFunctions.DiffDays(DateTime.Now, a.DateCreated) && a.IsVisible == 1 select a).Count();
                var TotalLoans = (from a in db.NyscLoanApplications where a.IsVisible == 1 select a).Count();

                float AppDisburse = AppDisburseds;
                float AppSubmitteds = TotalLoans;
                float Result = AppDisburse / AppSubmitteds;
                var Results = Math.Round(Result,3);
        ...
        public float GetDropOutRate()
        {
            try
            {
                ...
                float Result = AppDisburse / AppSubmitteds;
                Result = Result / 100;
                // var Total = AppDisburseds / TotalLoans * 100;

                var  Results = Math.Round((double)Result, 3);
        ...
        public string KpiLoanAppToRemitaGeneration(string RefNum)
        {
            ...
            try
            {
                var LoanCreateDate = (from a in db.NyscLoanApplications where a.RefNumber == RefNum select a).FirstOrDefault();
                var RemitaGenerated = (from b in db.PatnerTransactLogs where b.RefNum == RefNum select b).FirstOrDefault();
            ...
            try
            {
                ...
                foreach (var i in invoice)
                {
                    ...
                    var Lr = LedgRec.GroupBy(x => x.ReferenceNum).Select(g => new Invoice
                    {
                        ...
                        AmountPaid = (from x in uvDb.NYSCLoanLedgers where x.ReferenceNumber == i.ReferenceNum && x.Credit == 0 select x.Debit).Sum().Value,
                    }).FirstOrDefault();
       ...
       
        {
            try
            {
                ...
                Repayment Lt = new Repayment();
                TableObjects.LoanApplication LoanApps = new TableObjects.LoanApplication();
                //var LoanAps = new AppLoan();
                //AppLoan Apploan = new AppLoan();
                NyscLoanApplication LoanAp = new NyscLoanApplication();


                DataAccessA.DataManager.NYSCLoanLedger LoanLdger = new NYSCLoanLedger();
                DataAccessA.DataManager.NYSCLoanLedger LoanLdgers = new NYSCLoanLedger();
                ...
                if (ItemList.Length > 0)
                {
                    for (var m = 0; m < ItemList.Length; m++)
                    {
                        ...
                        var LoanLedg = _DM.InsertLedgerTransact(IDv, LoanLdger, Lt, Ampd);
                        ...
                        {
                            LoanLedg.PaymentFlag = 1;
                            LoanLedg.Debit = LoanLedg.Credit;
                            LoanApxs = _DM.UpdateLedger(IDv, LoanLedg);

                        }
                        if (LoanLedg.Credit > paidAmt && Ampd != 0)
                        {
                            LoanLedg.PaymentFlag = 4;
                            if (LoanLedg.Debit == null)
                            {
                                LoanLedg.Debit = 0;
                            }
                            LoanLedg.Debit += Ampd;
                        ...
                        {
                            LoanLedg.PaymentFlag = 1;
                            LoanLedg.Debit = LoanLdger.Credit;
                            LoanApxs = _DM.UpdateLedger(IDv, LoanLedg);
                        }
        ...
        //                {
        //                    LoanLedg.PaymentFlag = 1;
        //                    LoanLedg.Credit = LoanLdger.Debit;
        //                    LoanApxs = _DM.UpdateLedger(IDv, LoanLedg);
        //                }
        ...
        //[HttpPost]
        //public ActionResult ConfirmPays(DataAccessA.Classes.AppLoans LP, FormCollection form, string[] ItemList, float[] AmtPaid)
        //{
        //    try
        //    {
        //        GetMenus();
        //        TempData["SucMsg"] = ""; TempData["ErrMsg"] = "";
        //        var channellist = Request["Check"];
        //       // string[] ItemList = channellist.Split(',');

        //        // NYSCLoanLedgerTransact Lt = new NYSCLoanLedgerTransact();
        //        Repayment Lt = new Repayment();
        //        TableObjects.LoanApplication LoanApps = new TableObjects.LoanApplication();
        //        //  var LoanAps = new AppLoan();
        //        //  AppLoan Apploan = new AppLoan();

        //        var LoanAps = new DataAccessA.Classes.AppLoans();
        //        AppLoans Apploans = new AppLoans();

        //        NyscLoanApplication LoanAp = new NyscLoanApplication();

        //        NYSCLoanLedger LoanLdgers = new NYSCLoanLedger();
        //        NYSCLoanLedger LoanLdger = new NYSCLoanLedger();
        //        data dt = new data();
        //        int LoanApxs = 0; //int ID = 0;
        //        double Ledgertransact = 0; int vals = 0;
        //        if (ItemList == null)
        ...
        //                var paidAmt = 0;
        //                // if(Ledgertransact > Ampd)
        //                if (Ampd == 0 || Ampd >= LoanLdger.Credit)
        //                {
        //                    LoanLedg.PaymentFlag = 1;
        //                    LoanLedg.Debit = LoanLedg.Credit;
        //                    LoanApxs = _DM.UpdateLedger(IDv, LoanLedg);
        ...
        //            var invoice = _DR.getInvoices(LP.Enddate);
        ...
        [HttpPost]
        public ActionResult ConfirmPayment(DataAccessA.Classes.AppLoans lApObj)
        {
            ...
            try
            {
                ...
                var invoice = _DR.getInvoices(DOB);

                //var vav = GetPastInvoice(invoice, DOB);
       ...
       // Final Approval Here
        ...
        public int insertRepayment( DataAccessA.Classes.AppLoanss Apploans)
        {
            try
            {
                ...
                NYSCLoanSetUp NS = new NYSCLoanSetUp();
                ...
                //NL.ManagementFee = NS.ManagementFee;
                //NL.InterestAmount = NS.MonthlyInterestAmount;
...

        {
            try
            {
                ...
                string RefNum =GetRefNum(Refid);
               // RefNum = "NY15828993078798";
                var cd = KpiLoanAppToRemitaGeneration(RefNum);
                TempData["cd"] = cd;
        ...
        public string GetRefNum(int id)
        {
            try
            {
                var RefNum = (from a in db.NyscLoanApplications where a.ID == id select a.RefNumber).FirstOrDefault();
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (refNum != null || refNum != "")
                    {
                        ...
                        //jobj.requestId = rec.PatnerReference;//Convert.ToString(rec.requestId);
                        jobj.mandateId = "160386148950";//rec.mandateID;//Convert.ToString(rec.MandateId);
                        jobj.requestId = "1583765067792";//rec.PatnerReference;//Convert.ToString(rec.requestId);
                    }
                    else
                    {
                        jobj.mandateId = Convert.ToString(form["mandateId"]);
                    ...
                    var url = ConfigurationManager.AppSettings["MandateStatusLive"];
                    string data = Helper.DoPosts(url, "", "", "", "", "", "", json);
                    if (data == null)
                    {
                        TempData["ErrMsg"] = "Please Try Again";
                        return View();
        ...
        public ActionResult InitialDebit(FormCollection form, string Refid)
        {
            try
            {
                ...
                else
                {
                    ...
                    var url = ConfigurationManager.AppSettings["DirectDebitTest"];
                    string data = Helper.DoPost(url, "", "", "", "", "", "", json);
                    if (data == null)
                    {
                        TempData["ErrMsg"] = "Please Try Again";
                    ...
                    if (jObj.respCode != null || jObj.respCode != "")
                    {
                        ...
                        if (jObj.respCode == "00")
                        {
                            var reqID = jObj.requestID;
                            _DR.UpdateRequestIDFromInitialDebit(Refid, reqID);
                        }
                    }
                    else
                    {
        ...
        public List<Invoice> GetDuePaymentsByMonth(int mnth)
        {
            try
            {
                var LoansDue = (from s in db.NYSCLoanLedgers
                                join v in db.PatnerTransactLogs on s.ReferenceNumber equals v.RefNum
                                where s.DueDate.Value.Month == mnth && v.PatnerCode == "Remita" && s.ReferenceNumber.StartsWith("NY")

                                select new Invoice
                                {
                                    Debit = (double)s.Debit,
                                    ReferenceNum = s.ReferenceNumber,
                                    Company = v.MandateID,
                                    Location = v.BankCode,
                                    EmployeeID = v.BankAcct,
                                    AmountPaid = (double)s.Credit
        ...
        public ActionResult MonthlyRepayment(FormCollection form)
        {
            try
            {
                ...
                var LoansDue = GetDuePaymentsByMonth(monthID);
                ...
                if (LoansDue != null)
                {
                    if (LoansDue.Count > 0)
                    {
                        for (var w = 0; w <= LoansDue.Count; w++)
                        {
                            ...
                            if (mObj.respCode != "00")
                            {
                                // lblMsg.Text = " = " + lblMsg.Text + " - " + refNumber;
                                TempData["SucMsg"] = obj.refNumber;
                            }
                        }
        ...
        public ActionResult CBIndividualBorower()
        {
            ...
            try
            {
                ...
                AppStatFk = 1;
                userid = _DR.getUserID(user);
                ViewBag.Data = _DR.GetIndividualBorrower(AppStatFk);
                Session["AllTransaction"] = ViewBag.Data;
            ...
            try
            {
                ...
                if (rec.ID != 0 && rec != null)
                {
                    ...
                    if (jObj.respCode == "00")
                    {
                        ...
                        var DiffDate = LoanApp.DateModified.Value.Subtract(rec.DateCreated.Value);
